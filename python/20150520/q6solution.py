...
def separate(ll,p):
    ...
    # while ll != None:
    #     if p(ll.value):
    #         ll_true = insertFromHead(ll_true, ll.value)
    #     else:
    #         ll_false = insertFromHead(ll_false, ll.value)
    #     ll = ll.next
    # return ll_true, ll_false

    if ll != None:
        ll_true, ll_false = separate(ll.next, p)
        if p(ll.value):
            ll_true = insertFromHead(ll_true, ll.value)
        else:
            ll_false = insertFromHead(ll_false, ll.value)
...
def is_min_heap(t):
    ...
    if not t.left:
        return True
    elif t.left.value < t.value:
        return False
    if not t.right:
        return True
    elif t.right.value < t.value:
        return False
...
class StringVar_WithHistory(StringVar):
    ...
    def undo (self):
        if len(self.history) > 1:
            self.history.pop()
            return self._tk.globalsetvar(self._name, self.history[-1])
